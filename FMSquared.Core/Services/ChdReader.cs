...
/// <summary>
/// P/Invoke wrapper around libchdr for reading CHD disc images.
/// </summary>
public sealed class ChdReader : IDisposable
{
    ...
    public ChdHeader Header { get; private set; } = null!;
    public IReadOnlyList<ChdTrack> Tracks { get; private set; } = null!;
    ...
    public ChdReader(string filePath)
    {
        ...
        if (error == ChdError.RequiresParent)
            throw new InvalidOperationException(
                "This CHD file requires a parent CHD. Delta/diff CHDs are not supported.");
        if (error != ChdError.None)
            throw new InvalidOperationException(
                $"Failed to open CHD file: {ChdErrorString(error)} ({error})");
    ...
    private void ReadHeader()
    {
        ...
        if (headerPtr == IntPtr.Zero)
            throw new InvalidOperationException("Failed to read CHD header");
        ...
        if (Header.Version < 1 || Header.Version > 5)
            throw new InvalidOperationException($"Unsupported CHD version: {Header.Version}");
        if (Header.HunkBytes == 0)
            throw new InvalidOperationException("Invalid CHD: hunkbytes is 0");
    }
    ...
    private void ReadTracks()
    {
        ...
        // Try CD-ROM metadata tags in priority order.
        if (!TryReadTracksWithTag(CDROM_TRACK_METADATA2_TAG, tracks))
        {
            if (!TryReadTracksWithTag(CDROM_TRACK_METADATA_TAG, tracks))
            {
                TryReadTracksWithTag(CDROM_OLD_METADATA_TAG, tracks);
            }
        }

        if (tracks.Count == 0)
            throw new InvalidOperationException(
                "No track metadata found in CHD file. This may not be a disc image.");
    ...
    public byte[] ReadSector(long sectorIndex)
    {
        ...
        try
        {
            ...
            if (error != ChdError.None)
                throw new InvalidOperationException(
                    $"Failed to read CHD hunk {hunkNum}: {ChdErrorString(error)}");
    ...
    private long GetIpBinSectorOffset()
    {
        ...
        // Skip pregap frames to reach the actual data start.
        foreach (var track in Tracks)
        {
            if (track.IsData)
                return currentSector + track.Pregap;

            currentSector += track.Frames + GetExtraFrames(track.Frames);
        }

        throw new InvalidOperationException("No data track found in CHD file");
    }
    ...
    public long GetTrackDataStartSector(int trackIndex)
    {
        ...
        {
            var t = Tracks[i];
            currentSector += t.Frames + GetExtraFrames(t.Frames);
        }
    ...
    internal static int GetExtraFrames(int frames)
    {
        return ((frames + TrackPadding - 1) / TrackPadding) * TrackPadding - frames;
    }

    private static string ChdErrorString(ChdError error)
    {
        try
        {
...
public class ChdTrack
{
    ...
    public string PregapType { get; set; } = string.Empty;
    public string PregapSubType { get; set; } = string.Empty;
