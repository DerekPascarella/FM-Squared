...
public partial class ManualUpdateDialog : Window
{
    ...
    public ManualUpdateDialog(string latestTag, string latestVersion, ManualUpdateReason reason)
    {
        ...
        if (reason == ManualUpdateReason.UnsupportedPlatform)
            ReasonText.Text = $"A new version of FM^2 ({latestVersion}) is available. Auto-update is not supported on this platform.";
        else
            ReasonText.Text = $"A new version of FM^2 ({latestVersion}) is available, but this release cannot be auto-updated.";
