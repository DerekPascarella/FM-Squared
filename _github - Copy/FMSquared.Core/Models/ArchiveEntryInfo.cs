...
public sealed class ArchiveEntryInfo
{
    ...
    /// <summary>
    /// Index into the archive's counted entry list, in enumeration order.
    /// </summary>
...
public static class ArchiveEntrySelection
{
    ...
    // (.img, .iso, .mdf, .bin, .sub) are never excluded outright because
    // they can be companions of the selected image.
    ...
    private static readonly string[] StandaloneImageExtensions = { ".cdi", ".iso", ".img", ".mdf", ".bin" };
    ...
    private static bool IsExcludedBesideSelection(string leaf, string selectedExtension, bool selectedIsStandalone)
    {
        ...
        // A .bin is CUE track data (or a bare image). Beside any other
        // selection it is another game's data or junk that the save-time
        // copy would put on the card.
        if (extension == ".bin" && selectedExtension != ".cue")
            return true;

        // A standalone .iso beside a manifest-anchored set is another game.
        // A selected cue may reference an .iso as track data, so it stays.
        if (extension == ".iso" && selectedExtension != ".cue")
            return true;
