...
public static class UpdateManager
{
    ...
    private const string DefaultRepo = "DerekPascarella/FM-Squared";
    private const string StagingDirName = "FMSquared_update";
    private const string AutoUpdateKillSwitch = "This release cannot be auto-updated.";
    private const string WindowsScriptName = "_fm2_updater.bat";
    private const string UnixScriptName = "_fm2_updater.sh";
    ...
    static UpdateManager()
    {
        ...
        _client.DefaultRequestHeaders.UserAgent.ParseAdd("FMSquared-UpdateCheck/1.0");
    }
    ...
    private static string GetAssetUrl(string tag)
    {
        ...
        return $"https://github.com/{repoPath}/releases/download/{tag}/FMSquared.v{cleanTag}-{suffix}";
    }
    ...
    public static async Task DownloadUpdateAsync(string tag, IProgress<DownloadProgress>? progress, CancellationToken cancellationToken)
    {
        ...
        var suffix = GetAssetSuffix();
        var cleanTag = tag.TrimStart('v', 'V');
        var fileName = $"FMSquared.v{cleanTag}-{suffix}";
    ...
    {
        return File.Exists(Path.Combine(dir, "FMSquared.exe")) ||
               File.Exists(Path.Combine(dir, "FMSquared")) ||
               File.Exists(Path.Combine(dir, "FMSquared.dll"));
    }
    ...
    private static void MergeSettings(string extractedDir)
    {
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            ...
            currentSettingsPath = Path.Combine(home, "Library", "Application Support", "FMSquared", "settings.json");
        }
...
start """" ""{Path.Combine(escaped_app, "FMSquared.exe")}""
...
# Fix permissions
chmod +x ""{escaped_app}/FMSquared""

# Relaunch the app
""{escaped_app}/FMSquared"" &
